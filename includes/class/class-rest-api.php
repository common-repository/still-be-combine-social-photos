...
final class Rest_API {
	...
	public static function get_user_media( \WP_REST_Request $request ) {
		...
		// ID is not Found...
		if( empty( $account->token->token ) && empty( $account->token->token ) || empty( $account->token->api ) || empty( $account->me->id ) ) {
	...
	public static function convert_hashtag( \WP_REST_Request $request ) {
		...
		// ID is not Found...
		if( empty( $account->token->user_token ) || empty( $account->token->api ) || empty( $account->me->id ) ) {
		...
		$hashtag_id = $getting_api::hashtag_search( $account->me->id, $account->token->user_token, $hashtag );
		...
		if( empty( $hashtag_id ) ) {
			$response = new \WP_REST_Response( array(
				...
				'test'    => [$hashtag_id,$account->me->id, $account->token->user_token, $hashtag],
			) );
		} else {
			$response = new \WP_REST_Response( array(
				'ok'         => true,
