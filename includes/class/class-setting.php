...
final class Setting {
	...
	public function render_setting_page() {
		...
		// Curernt Settings
		$this->settings = empty( $this->settings ) ? get_option( self::SETTING_NAME, array() ) : $this->settings;
		...
		echo '<div class="wrap">';
			...
			// Other Products
			Admin_Notice::init()->other_products_show();
	...
	public function display_notices() {
		...
		foreach( $accounts as $account ) {
			...
			if( empty( $account->id ) || empty( $account->api_type ) || 'Graph API' !== $account->api_type ) {
				continue;
			}

			if( empty( $account->token->user_token )
			      || ( ! empty( $account->token->user_expire ) && 24 * 3600 > $now - $account->token->user_expire )
			    ) {
				$no_user_token_ids[] = $account->id;
			}

		}

		if( ! empty( $no_user_token_ids ) ) {

			add_settings_error(
				$group_name,
				self::PREFIX. 'getting_user_token_notice',
				(
					esc_html__( sprintf( 'No user token is set for ID = %s.', implode( ', ', $no_user_token_ids ) ), 'still-be-combine-social-photos' ).
					'<br>'.
					esc_html__( 'A user token is now required to show hashtag posts. Please use the "Reauth Token" button to re-authenticate your account.', 'still-be-combine-social-photos' )
				),
				'warning'
			);
		...
		// Show Donate Links
		Admin_Notice::init()->donate_links_show();
	...
	//	echo '</pre>';
		...
		foreach( $accounts as $account ) {
			...
			$refresh_token = true;
			if( empty( $account->token->token ) || ( ! empty( $account->token->expire ) && ( empty( $account->token->created ) || 24 * 3600 > $account->token->created - $now ) ) ) {
				$refresh_token = false;
			}
			if( empty( $account->me ) ) {
				$refresh_token = true;
			...
			if( isset( $account->token->expire ) ) {
				if( 0 == $account->token->expire ) {
					...
					$expire = '<span>'. esc_html__( 'No expiration date', 'still-be-combine-social-photos' ). '</span>';
				} else {
					$expire = '<span>'. wp_date( __( 'Y-m-d H:i P (e)', 'still-be-combine-social-photos' ), absint( $account->token->expire ) ).
					            '<br>'.  esc_html__( '* It will be automatically updated.', 'still-be-combine-social-photos' ). '</span>';
				}
			} else {
				$expire = '<span>'. __( 'Getting...', 'still-be-combine-social-photos' ). '</span>';
			}

			if( isset( $account->token->user_expires ) ) {
				if( 0 == $account->token->user_expires ) {
					$expire .= '<span>'. __( 'No expiration date', 'still-be-combine-social-photos' ). '</span>';
				} else {
					$expire .= '<span>'. wp_date( __( 'Y-m-d H:i P (e)', 'still-be-combine-social-photos' ), absint( $account->token->user_expires ) ). '</span>';
				}
			}
			...
			$is_no_user_token = false;
			if( isset( $account->api_type ) && 'Graph API' === $account->api_type ) {
				if( empty( $account->token->user_token ) ) {
					$is_no_user_token = true;
				}
				if( ! isset( $account->token->user_expires ) || ( 0 < $account->token->user_expires && 24 * 3600 > $account->token->user_expires - $now ) ) {
					$is_no_user_token = true;
				}
			}
			...
			echo     '<div class="button-wrapper">';
			if( $is_disabled_token || $is_no_user_token ) {
		...
		echo           '</div>';
		echo           '<div>';
		echo             '<dt>'. esc_html__( 'Access Token for Page', 'still-be-combine-social-photos' ). '</dt>';
		echo             '<dd>';
		...
		echo           '</div>';
		echo           '<div>';
		echo             '<dt>'. esc_html__( 'Expires in for Page', 'still-be-combine-social-photos' ). '</dt>';
		echo             '<dd>';
		...
		echo           '</div>';
		echo           '<div>';
		echo             '<dt>'. esc_html__( 'Access Token for User', 'still-be-combine-social-photos' ). '</dt>';
		echo             '<dd>';
		echo               '<textarea name="data[user_token]" required></textarea>';
		echo               '<p class="note">';
		echo                 '<small>'. esc_html__( '* For Graph API, enter a page token that has not expired..', 'still-be-combine-social-photos' ). '</small><br>';
		echo               '</p>';
		echo             '</dd>';
		echo           '</div>';
		echo           '<div>';
		echo             '<dt>'. esc_html__( 'Expires in for User', 'still-be-combine-social-photos' ). '</dt>';
		echo             '<dd>';
		echo               '<input type="number" name="data[user_expires]">';
		echo               '<p class="note">';
		echo                 '<small>'. esc_html__( '* Enter a timestamp (integer value).', 'still-be-combine-social-photos' ). '</small><br>';
		echo                 '<small>'. esc_html__( '* If unknown, leave empty and refresh the token from the linked account table after registration.', 'still-be-combine-social-photos' ). '</small>';
		echo               '</p>';
