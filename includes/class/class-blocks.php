...
final class Blocks {
	...
	public function admin_enqueue_scripts( $hook_suffix ) {
		...
		// Block; Masonry Grid - common.css
		wp_register_style(
			'combine-social-photos-masonry-grid-style',
			STILLBE_CSP_BASE_URL. '/blocks/masonry-grid/common.css',
			[],
			@filemtime( STILLBE_CSP_BASE_DIR. '/blocks/masonry-grid/common.css' )
		);

		// Block; Masonry Grid - editor.css
		wp_register_style(
			'combine-social-photos-masonry-grid-editor',
			STILLBE_CSP_BASE_URL. '/blocks/masonry-grid/editor.css',
			[],
			@filemtime( STILLBE_CSP_BASE_DIR. '/blocks/masonry-grid/editor.css' )
		);

		// Block; Masonry Grid - editor.js
		wp_register_script(
			'combine-social-photos-masonry-grid-editor',
			STILLBE_CSP_BASE_URL. '/blocks/masonry-grid/editor.js',
			[ 'wp-blocks', 'wp-block-editor', 'wp-element', 'wp-components', 'wp-i18n' ],
			@filemtime( STILLBE_CSP_BASE_DIR. '/blocks/masonry-grid/editor.js' )
		);

		// Block; Masonry Grid - layout.js
		wp_register_script(
			'combine-social-photos-masonry-grid-layout',
			STILLBE_CSP_BASE_URL. '/blocks/masonry-grid/layout.js',
			[],
			@filemtime( STILLBE_CSP_BASE_DIR. '/blocks/masonry-grid/layout.js' )
		);

		// Add JS Translate
		Main::add_admin_js_translate_handles( 'combine-social-photos-masonry-grid-editor' );
	...
	public function register_blocks() {
		...
		// Masonry Grid
		register_block_type( self::BLOCKS_DIR. '/masonry-grid', array(
			'render_callback' => [ $this, 'render_masonry_grid' ],
		) );
	...
	//	$business_discovery = empty( $attributes['businessDiscovery'] ) ? null : trim( $attributes['businessDiscovery'] );
		 ...
		 */
		$access_token = empty( $hashtag ) ? $account->token->token : ( $account->token->user_token ?? null );
		$media_data   = $getting_api::get_media_data( $account->me->id, $access_token, $get_media_count, [], $advanced, empty( $content ) );
		...
		return apply_filters( 'stillbe_csp/common__dynamic_html', $html );
	...
	public function render_masonry_grid( $attributes, $content ) {

		$id = $attributes['id'] ?? 0;

		wp_enqueue_script( 'combine-social-photos-common-front' );
		wp_enqueue_script( 'combine-social-photos-masonry-grid-layout' );
		...
		// Media Count
		$media_count = $attributes['mediaCount'] ?? 10;

		// Getting API
		$get_media_count = $media_count;   // max( $media_count_pc, $media_count_tablet, $media_count_sp );
		$getting_api     = self::_get_api_class( $account->api_type ?? $account->token->api );
	...
	//	$business_discovery = empty( $attributes['businessDiscovery'] ) ? null : trim( $attributes['businessDiscovery'] );
		...
		$template = apply_filters( 'stillbe_csp/masonry_grid__template_path', STILLBE_CSP_BASE_DIR. '/templates/masonry-grid.php' );
		$html = require( $template );
